...
class App extends React.Component {
    ...
    filterProjects(filter) {
        ...
        axios.get('http://127.0.0.1:8000/api/projects/', {headers, params: {'title': filter}})
            .then(response => {
    ...
    createProject(title, description, repo_link, users) {
        const headers = this.getHeaders()
    ...
    createTodo(title, description, project, user) {
        const headers = this.getHeaders()
                ...
                <BrowserRouter>
                    <nav className="navbar navbar-expand-lg navbar-light bg-light">
                        ...
                        {this.isAuthenticated() ?
                            <span>{this.state.username}
                                <button onClick={() => this.logout()}>Выйти</button></span> :
                    ...
                    <content>
                        <Switch>
                            ...
                            <Route path='/projects/create' component={() => <ProjectForm users={this.state.users}
                                                                                         createProject={(title, description, repo_link, users) => this.createProject(title, description, repo_link, users)}/>}/>
                            <Route path='/todo/create'
                                   component={() => <TodoForm projects={this.state.projects} users={this.state.users}
                                                              createTodo={(title, description, project, user) => this.createTodo(title, description, project, user)}/>}/>
